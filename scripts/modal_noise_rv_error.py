...
def main(folder=FOLDER, cameras=CAMERAS, num_images=NUM_IMAGES, new_data=NEW_DATA, meth=METHOD, plot_fiber_centroid=PLOT_FIBER_CENTROID):
    for cam in cameras:
        ...
        if plot_fiber_centroid:
            ...
            if num_images == 1:
                ...
                for i in xrange(0, 300, 10):
                    ...
                    center_avg_xlist.append(np.average(center_x_to_avg))
                    center_avg_ylist.append(np.average(center_y_to_avg))
                    centroid_avg_xlist.append(np.average(centroid_x_to_avg))
                    centroid_avg_ylist.append(np.average(centroid_y_to_avg))
                    itr = [x + 10 for x in itr]
        ...
        if plot_fiber_centroid:
            ...
            ax1.set_ylabel('Center X drift ($\mu m$)')
            ax2.set_ylabel('Center Y drift ($\mu m$)')
            ax3.set_ylabel('Centroid X drift ($\mu m$)')
            ax4.set_ylabel('Centroid Y drift ($\mu m$)')
            ax4.set_xlabel('Frame number')

        if plot_fiber_centroid:
            if num_images == 1:
                fig1.legend((drift_xline, avg_xline), ('$\sigma_x=%.2f$\n$\sigma_y=%.2f$\n$\sigma_t=%.2f$' % (std_x, std_y, sig_xy), '$\sigma_x%.2f$\n$\sigma_y=%.2f$\n$\sigma_t=%.2f$' % (std_x_avg, std_y_avg, std_xy_avg)), loc='lower center')
            else:
                fig1.legend(drift_xline, '$\sigma_x=%.2f$\n$\sigma_y=%.2f$\n$\sigma_t=%.2f$' % (std_x, std_y, sig_xy), loc='lower center')
        else:
            if num_images == 1:
                fig1.legend((center_xline, center_avg_xline, centroid_xline, centroid_avg_xline), ('Center\n$\sigma_x=%.2f$\n$\sigma_y=%.2f$\n$\sigma_t=%.2f$' % (center_std_x, center_std_y, center_sig_xy), 'Average of center\n$\sigma_x=%.2f$\n$\sigma_y=%.2f$\n$\sigma_t=%.2f$' % (center_sig_x_avg, center_sig_y_avg, center_sig_xy_avg), 'Centroid\n$\sigma_x=%.2f$\n$\sigma_y=%.2f$\n$\sigma_t=%.2f$' % (centroid_std_x, centroid_std_y, centroid_std_xy), 'Average of centroid\n$\sigma_x=%.2f$\n$\sigma_y=%.2f$\n$\sigma_t=%.2f$' % (centroid_std_x_avg, centroid_std_y_avg, centroid_std_xy_avg)), loc='lower center')
            else:
                fig1.legend((center_xline, centroid_xline), ('Center\n$\sigma_x=%.2f$\n$\sigma_y=%.2f$\n$\sigma_t=%.2f$' % (center_std_x, center_std_y, center_sig_xy), 'Centroid\n$\sigma_x=%.2f$\n$\sigma_y=%.2f$\n$\sigma_t=%.2f$' % (centroid_std_x, centroid_std_y, centroid_std_xy)), loc='lower center')
